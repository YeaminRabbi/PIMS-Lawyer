                ...
                @foreach ($blog as $data)
                    <div class="col-md-4 d-flex ftco-animate">
                        <div class="blog-entry justify-content-end">
                            <a href="{{ route('BlogDetails', $data->id)}}" class="block-20" style="background-image: url({{ asset('images/blog/'.$data->created_at->format('Y/M/').'/'.$data->image) }});">
                            </a>
                            ...
                            <a href="{{ route('BlogDetails', $data->id)}}">
                                <h4 class="heading text-justify">{{$data->title}}</h4>
                            </a>
                            <p>{!! Str::limit($data->introduction, 100,'.....') !!}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
